...
buscarUm: (paci_id) => {
    return new Promise((aceito, recusado) => {
        db.query('SELECT * FROM paciente WHERE paci_id = ?', [paci_id], (error, results) => {
            ...
            aceito(results[0]); // Retornar apenas o primeiro resultado, que deve ser o paciente único com o paci_id fornecido
        });
    });
},
...
inserir: (paci_nome, paci_data_nascimento, paci_CPF, paci_cel, paci_email, paci_endereco, paci_senha) => {
    return new Promise((aceito, recusado) => {
        db.query('INSERT INTO paciente (paci_nome, paci_data_nascimento, paci_CPF, paci_cel, paci_email, paci_endereco, paci_senha) VALUES (?,?,?,?,?,?,?)',
...
alterarDadosPaciente: (paci_id, dados) => {
    return new Promise((aceito, recusado) => {
        const fieldsToUpdate = []; // Array para armazenar os campos a serem atualizados
        const valuesToUpdate = []; // Array para armazenar os valores dos campos a serem atualizados

        // Iterar sobre os dados fornecidos e adicionar apenas os campos definidos
        ...
        // Adicionar o ID do paciente ao final do array de valores
...
excluir: (paci_id) =>{        // -------------------------EXCLUIR DADOS--------------------------------- //
  ...
  verificarLogin: (paci_CPF, paci_senha) => {
    return new Promise((aceito, recusado) => {
      db.query(
        'SELECT * FROM paciente WHERE paci_CPF = ? AND paci_senha = ?',
        [paci_CPF, paci_senha],
        (error, results) => {
          if (error) {
          ...
          // Verifica se há algum resultado retornado da consulta
          if (results.length > 0) {
